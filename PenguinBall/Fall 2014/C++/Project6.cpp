...
public:
    ...
    bool MarketTransactionBuying(int orderPrice, char indexType,int& holdingsB
                    ,int& holdingsF, int& holdingsI,int& valuesB, int& valuesF,
                     int& valuesI, int& finalMMCash);
    bool MarketTransactionSelling(int orderPrice, char indexType,int& holdingsB,
                                  int& holdingsF, int& holdingsI,int& valuesB,
                                  int& valuesF, int& valuesI, int& finalMMCash);
    ...
    //Takes no inputs. Returns how many units of an MM property is owned.
        //Takes no inputs. Nothing is returned but the MM Buy Order is "withdrawn" and then changed to a new value.
    void AddMMBuyOrder(int order){
        MMBuyOrder = order;
    }
    //Takes no inputs. Nothing is returned but the MM Sell Order is "withdrawn" and then changed to a new value.
    void AddMMSellOrder(int order){
        MMSellOrder = order;
    }
    //accessor functions
    //Takes no inputs. Returns what is the maximum order in the buy book which is stored at the first location since it is organized in ascending order.
    int GetMaxBuy() const{
        return buyOrders.at(0);
    }
    //Takes no inputs. Returns what is the minimum order in the sell book which is stored at the first location since it is organized in descending order.
    int GetMinSell() const {
        return sellOrders.at(0);
    }
    //Takes no inputs. Returns the buy order currenty placed by the Market Making strategy. If no order exists it returns zero.
    int GetMMBuyOrder() const{
        return MMBuyOrder;
    }
    //Takes no inputs. Returns the sell sorder currenty placed by the Market Making strategy. If no order exists it returns zero.
    int GetMMSellOrder() const{
        return MMSellOrder;
    }
    //derived classes have access to these vectors
protected:
...
public:
    IndexArbitrage();
    //mutator functions
    //Takes inputs for the minimun and maximum repestive values needed to sell an Index and make profit. We use the boolean values to determine if the inputs are from the MM index or not and thus treat each output correctly. Finally we have passed by reference the values used to store the total profits so they can be altered wihtouth having to be returned. Returns nothing.
    void sellIndex(int sugarBombsMinSell, int flakeyPopsMinSell,
                   int bothCerealsMaxBuy, bool sugarIsMM, bool flakeyIsMM,
                   bool bothCerealsIsMM, int& holdingsB, int& holdingsF,
                   int& holdingsI, int& valuesB, int& valuesF, int& valuesI,
                   int& finalMMCash){
        if (sugarIsMM == true){
       ...
       //Takes inputs for the minimun and maximum repestive values needed to buy an Index and make profit. We use the boolean values to determine if the inputs are from the MM index or not and thus treat each output correctly. Finally we have passed by reference the values used to store the total profits so they can be altered wihtouth having to be returned. Returns nothing.
    void buyIndex(int sugarBombsMaxBuy, int flakeyPopsMaxBuy,
                  int bothCerealsMinSell, bool sugarIsMM, bool flakeyIsMM,
                  bool bothCerealsIsMM, int& holdingsB, int& holdingsF, int& holdingsI,
                  int& valuesB, int& valuesF, int& valuesI, int& finalMMCash){
        if (sugarIsMM == true){
    ...
    //accessor function
    // Takes no input. Returns the total profit made using the index arbitrage method.
    int GetFinalProfit() const{
        return totalProfit;
    }
private:
    int roundProfit;
...
//This class keeps track of profits and earnings gotten from the Market Making Strategy and thus how many of each type of a unit are purchased the FINAL transaction value for each of them.
class MarketMaking : public CerealMarkets {
public:
    MarketMaking();//default constructor
    //accessor methods
    //Takes no input returns how many units of a good is owned.
    int GetUnitsOwned() const{
        return unitsOwned;
    }
    //Takes no inputs. Returns the last value a cereal was transacted at.
    int GetUnitValues() const{
        return unitValues;
    }
    //mutator methods
    //Takes no inputs. Nothing is returned but one additional unit of MM property is added.
    void AddUnitOwned(){
        unitsOwned++;
    }
    //Takes no inputs. Nothing is returned but one unit of MM property is removed.
    void RemoveUnitOwned(){
        unitsOwned--;
    }
    //Takes the most recent transaction value as an input and then stores it as the value for that cereal. Returns nothing.
    void SetUnitValues(int price){
        unitValues = price;
    }
...
//Default Constructors: initialize all values to zero. Take no inputs and return no values.
...
bool CerealMarkets::MarketTransactionBuying(int orderPrice, char indexType,
                                            int& holdingsB, int& holdingsF, int& holdingsI,
                                            int& valuesB, int& valuesF, int& valuesI, int& finalMMCash) {
 ...
 //Takes the selling request price and index type(which market we are operating on) as inputs. Uses the selling request price to see if it is less than or equal to the largest buying request(stored at 0 since its in descending order). If this is true transaction occurs otherwise it doesn't.
            bool CerealMarkets::MarketTransactionSelling(int orderPrice,
                char indexType, int& holdingsB, int& holdingsF, int& holdingsI,
                int& valuesB, int& valuesF, int& valuesI, int& finalMMCash) {
    ...
    while(inFS >> indexType) {
        ...
        if (indexType == 'B') {
            if (transactionType == 1) {
                if (sugarBombs.MarketTransactionBuying(orderPrice,
                        indexType,holdingsB, holdingsF, holdingsI, valuesB,
                        valuesF, valuesI, finalMMCash) == true) {
                sugarBombs.AddBuyOrders(orderPrice);
                }
            }
            if (transactionType == -1) {
                if (sugarBombs.MarketTransactionSelling(orderPrice,
                        indexType,holdingsB, holdingsF, holdingsI, valuesB,
                        valuesF, valuesI, finalMMCash) == true) {
        ...
        if (indexType == 'F') {
            if (transactionType == 1) {
                if (flakeyPops.MarketTransactionBuying(orderPrice,indexType,
                    holdingsB, holdingsF, holdingsI, valuesB,
                    valuesF, valuesI, finalMMCash) == true) {
                flakeyPops.AddBuyOrders(orderPrice);
                }
            }
            if (transactionType == -1) {
                if (flakeyPops.MarketTransactionSelling(orderPrice,indexType,
                    holdingsB, holdingsF, holdingsI, valuesB, valuesF,
                    valuesI, finalMMCash) == true) {
        ...
        if (indexType == 'I') {
            if (transactionType == 1) {
                if (bothCereals.MarketTransactionBuying(orderPrice,indexType,
                    holdingsB, holdingsF, holdingsI, valuesB,
                    valuesF, valuesI, finalMMCash) == true) {
            ...
            if (transactionType == -1) {
                if (bothCereals.MarketTransactionSelling(orderPrice,indexType,
                    holdingsB, holdingsF, holdingsI, valuesB,
                    valuesF, valuesI, finalMMCash) == true) {
        ...
        if (flakeyPops.GetMaxBuy() !=0 && sugarBombs.GetMaxBuy()
            !=0 && bothCereals.GetMinSell() != 0) {
            ...
            if ((bothCereals.GetMinSell() > bothCereals.GetMMSellOrder())
                && (bothCereals.GetMMSellOrder() != 0)){
        ...
        if ((maxFlakeyBuy + maxSugarBuy) > minBothSell) {
            arbitrator.buyIndex(maxSugarBuy, maxFlakeyBuy, minBothSell,
            maxSugarBuyIsMM, maxFlakeyBuyIsMM, minBothSellIsMM, holdingsB,
            holdingsF, holdingsI, valuesB, valuesF, valuesI, finalMMCash);
        ...
        //first if statement confirms that the bid and ask quotes in the scenarios actually exist
        //second if statement checks to see if transaction would be profitable and whether or not to proceed
        if (flakeyPops.GetMinSell() !=0 && sugarBombs.GetMinSell()
            != 0 && bothCereals.GetMaxBuy() != 0){

            if ((flakeyPops.GetMMSellOrder() < flakeyPops.GetMinSell())
                && (flakeyPops.GetMMSellOrder() != 0)) {
            ...
            if ((sugarBombs.GetMMSellOrder() < sugarBombs.GetMinSell())
                && (sugarBombs.GetMMSellOrder() != 0)) {
        ...
        if (maxBothBuy > (minFlakeySell + minSugarSell)) {
            arbitrator.sellIndex(minSugarSell, minFlakeySell,
            maxBothBuy, minSugarSellIsMM, minFlakeySellIsMM, maxBothBuyIsMM,
            holdingsB, holdingsF, holdingsI,valuesB, valuesF, valuesI, finalMMCash);
        ...
        if (flakeyPops.GetMinSell() != 0 && flakeyPops.GetMaxBuy() != 0){
            ...
            if ((flakeyPops.GetMinSell() - flakeyPops.GetMaxBuy()) >= threshold){
                ...
                else if ((flakeyPops.GetMinSell() < flakeyPops.GetMMSellOrder())
                         or flakeyPops.GetMMSellOrder() == 0){
        ...
        if (sugarBombs.GetMinSell() != 0 && sugarBombs.GetMaxBuy() != 0){
            ...
            if ((sugarBombs.GetMinSell() - sugarBombs.GetMaxBuy()) >= threshold){
                ...
                if (sugarBombs.GetMinSell() < sugarBombs.GetMMSellOrder()
                    or sugarBombs.GetMMSellOrder() == 0){
        ...
        if (bothCereals.GetMinSell() != 0 && bothCereals.GetMaxBuy() != 0){
            ...
            if ((bothCereals.GetMinSell() - bothCereals.GetMaxBuy()) >= threshold){
                ...
                if (bothCereals.GetMinSell() < bothCereals.GetMMSellOrder()
                    or bothCereals.GetMMSellOrder() == 0){
                    bothCereals.AddMMSellOrder((bothCereals.GetMinSell())-1);
                }
            }
        }
    }
    //since we are evaluating the inputs as the are coming in the stream only after all the data comes in and is analyzed can we close the file
    inFS.close();
    
    cout << "IA final holdings: cash = "
    << arbitrator.GetFinalProfit() << endl;
    cout << "MM final holdings: cash = "
    << finalMMCash << endl;
    cout << "Security B: " << holdingsB << " units, value = "
    << holdingsB * valuesB << endl;
    cout << "Security F: " << holdingsF << " units, value = "
    << holdingsF * valuesF << endl;
    cout << "Security I: " << holdingsI << " units, value = "
    << holdingsI * valuesI << endl;
    cout << "Total profit is "
    << finalMMCash + (holdingsB * valuesB) +
    (holdingsF * valuesF) + (holdingsI * valuesI)
    << "." << endl;
