...
class GameScene: SKScene, SKPhysicsContactDelegate {
   ...
   // func UpDateScore() {
    ...
    func CreateNewObstacle() {
        ...
        case(5):
            var LogTexture4 = SKTexture(imageNamed: "icicles1.png")
            LogTexture4.filteringMode = SKTextureFilteringMode.Nearest
            log1 = SKSpriteNode(texture: LogTexture4);
            log1.setScale(0.5)
            log1.physicsBody?.dynamic = false;
            log1.physicsBody?.collisionBitMask = logCategory;
            log1.position = CGPoint(x:self.frame.size.width * 0.9, y: self.frame.size.height * 0.19)
    ...
    override func touchesBegan(touches: NSSet, withEvent event: UIEvent) {
        ...
        for touch: AnyObject in touches {
            ...
            ball.physicsBody?.applyImpulse(CGVectorMake(0, 17.0));
            }
